...
3. 手动「重建向量索引」当前是全量重建：会删除当前小说已有向量片段，再重新处理当前小说的角色、物品、世界设定、大纲、章节正文、章节摘要和叙事摘要。
4. 首次全量索引建立后，章节、角色、物品、世界设定、大纲线、情节点、章节摘要、叙事摘要会做 source 级增量更新：只删除/重建对应 `source_type + source_id` 的片段，不会重新发送所有章节。
5. 关系、地图标记等资料当前主要作为结构化证据实时读取，尚未单独向量化；它们变化后不需要重建向量索引也能进入非向量证据链路。
6. 一次提问时，如果索引表里已有当前小说的片段，就直接读取并计算相似度；如果没有片段，会自动触发一次索引构建。
7. 当前没有使用 `sqlite-vec`。检索方式是从 SQLite 读出片段向量后，在应用层做余弦相似度计算。
8. 最终证据不是只有向量检索结果，还会混合角色卡、关系、物品、地图标记、大纲情节点、章节摘要、叙事摘要、关键词搜索结果和当前章节上下文。
9. 证据会先去重，再按分数排序，最后重新编号为 `[E1]`、`[E2]`。
10. “高置信度”当前主要由回答内容和证据数量推断：如果回答没有表示资料不足，且证据数量达到阈值，就可能显示高置信度。

## 隐私与外发边界

1. 重建向量索引时，章节、角色、大纲、摘要等切片会发送给 Embedding API。
2. 如果 Embedding Base URL 是本机 Ollama 或本地服务，切片不离开本机。
3. 如果 Embedding Base URL 是云端服务，切片会外发到该云端 Embedding 服务。
4. 点击「提问」时，最终文本模型只收到筛选后的 Evidence，不会收到所有命中章节。
5. 如果当前文本模型是云端 API，Evidence block、问题和可选当前章节上下文会外发到该文本模型服务。
6. 如果当前文本模型也是本地服务，则问答阶段也不离开本机。
