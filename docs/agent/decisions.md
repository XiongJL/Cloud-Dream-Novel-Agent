...
102. Agent 聊天采用 Intent 优先、正文按需读取。首次 `agent.generate_chat` 只接收请求、角色、历史摘要和范围元数据；明确计划在正式执行前不得读取正文。稳定 Toolchain 由 IntentDecision 确定性生成内部 Plan，不先调用模型 Planner。单 Operation、单专家、当前章或显式 1–3 章、`detailed` 且声明 `auto_small_scope` 的只读链可以由 Runtime 策略批准并直接启动；整卷、整本、4 章以上、批处理、研究、团队、动态 Planner 和所有副作用任务继续确认。内部免确认 Plan 仍创建 Run、Artifact 和审计事件，Renderer 不展示形式化 PlanCard；Runtime 必须重新验证只读属性和成本阈值。
103. 章节单章和多章节草稿生成改为持久化异步 Operation。LangGraph 保留为上层编排与 checkpoint 框架，通过幂等 `start`、`waiting_operation` interrupt 和完成事件恢复执行；Electron Main 负责 Operation Worker、Provider attempt、取消、重试和草稿提交。`operationId` 是持久化业务身份，`operationKey` 不包含 `runId`；普通连接丢失或 Provider timeout 通过 Operation 状态恢复，不再默认升级为 `SIDE_EFFECT_UNKNOWN`。DraftSession、DraftBatch、DraftBatchChild 与 Operation 在同一 SQLite 中重新建模，并通过事务和唯一约束保证同一 generation revision 至多一个有效草稿。此次切换允许一次性删除升级前历史 Agent 会话、Run、草稿会话、草稿批次、调用账本和 Graph checkpoint，不实现 JSON 导入、双读或 legacy reconciliation reader；Novel、Volume、Chapter 及正式正文等创作数据不得删除。本次一次性切换是决策 23、44、97、100 对升级前存量数据的特例，不改变切换完成后的新会话、新 Run 和新产物保留规则。首期状态机使用纯 TypeScript 转换器、SQLite CAS、lease 和 outbox，不引入 Temporal、BullMQ 或 XState；达到嵌套/并行状态复杂度门槛后才单独评估 XState v5。完整契约见 [Agent 持久化异步草稿 Operation 需求与技术方案](./durable-draft-operation-requirements.md)。
104. `RevisionWorkBatch` 中的多章节拍确认是原位修订进度的生成前检查点，不是被决策 86 免除的普通形式化计划审批。任何当前 Run 进入 `waiting_approval` 时，对话必须存在且只能存在一个可提交入口，Inspector 折叠或专用 Renderer 不得阻断。`draftBatchId` 创建后立即提供阶段化只读入口：节拍待确认时“查看章节拍”、正文生成时“查看生成进度”、部分完成时“查看已生成内容”，只有正式批次产物就绪后才称为“打开批次审核”。自动化与人工验收必须覆盖 `帮我修改 -> waiting_approval/chapter_beats -> 确认 -> 生成 -> 批次审核` 的完整状态可达性。
105. 长会话 coverage 只表示连续较早前缀已由通过来源校验的持久语义摘要表示；本轮 `omitted`、低优先级 section 删除和任意字符串截断都不得推进 coverage。本决策覆盖决策 77 中“摘要或省略时增量覆盖”的省略部分。完整 `AgentMessage` 继续作为事实源，当前用户请求和未决输入/审批/计划/范围等结构化状态不可有损压缩；压缩采用近期原文尾部、压缩后预算回检、按会话/模型连续三次失败熔断和可引用长内容微压缩。领域摘要、v1/v2 迁移、失败降级及验收契约见 [Agent 上下文压缩与长会话连续性需求](./context-compression-requirements.md)。
106. 每个 `Run + draftBatchId` 在会话中只有一张当前批次主卡。章节拍检查点嵌入该卡，确认后原卡切换为生成进度，不再并列渲染“查看节拍”和“查看章节拍”，也不保留同级的“已确认节拍”大卡；已替代或已确认 revision 收入卡内折叠的“节拍记录”。Inspector 使用互斥的节拍快照、批次实时进度、未完成状态和正式审核模式，标题与数据源由显式模式决定；`queued / leased / running / waiting_operation` 不得推断为生成未完成。完整交互契约见 [Agent 多章节处理 V1 需求](./multi-chapter-processing-requirements.md#1142-单一稳定卡片与章节拍历史)。
107. Agent 恢复资格由持久化的阶段 checkpoint 与 `AgentRecoveryDescriptor.retryStrategy` 决定，不再以网络重试耗尽事件作为唯一条件。模型完整结果必须在严格解析、领域转换和 Artifact 发布前保存；后续本地失败优先复用结果重新处理或继续幂等发布，不得默认重调模型、重建计划或要求用户发送“重试”。Run、步骤与 Artifact 状态相互独立：父 Run 后续失败时，`ready` 且未过期的 Artifact 仍可查看、批注和执行其允许的审核操作；副作用结果未知仍只能对账。完整契约见 [Agent 运行韧性、重试、降级与任务恢复需求](./retry-failover-resilience-requirements.md)。
108. 旧 OpenClaw Skill 兼容层正式删除，包括 `openclaw.skill.v1` manifest、Skill invoke、IPC/preload 暴露和 Skill 冒烟诊断；OpenClaw Tool/MCP 路径继续保留。项目内“Agent Skill”统一指受 Runtime Policy 约束的声明式创作方法层，不再复用 OpenClaw action 的 Skill 命名。Skill 开发先完成影响代码形状的最小契约修订，再以 `builtin.continuity-review -> chapter.consistency_review -> chapter.consistency_review@1.0.0` 打通首个 AS-0 纵切，用户编辑器和持久化不阻塞该纵切。
109. Agent Skill 创建是正式产品能力，不要求用户手写 `SKILL.md`。自然语言、`/skill create` 和快捷按钮统一进入内部 Creator Profile `builtin.skill-creator + agent.skill.author`；该 Profile 是创建元能力和系统提示标识，不作为普通可选择 `AgentSkillDefinition` 注册。所有创建、更新、拆分和组合先形成可恢复 AgentSkillDraft，经来源覆盖、预算、安全、污染和试运行校验并由用户确认后才提交不可变 Revision。Creator 可以按授权读取当前对话、当前小说、附件、本地文档和公开 Web 来源；模型已有知识只能生成低置信度候选，Creator 的读取权限不继承到创建结果。输入框以 `/` 打开统一面板，分组列出系统动作、Built-in、User 和当前 Novel Skill/Pack，选择后转为结构化 Entry Hint 与芯片；`/skill list` 不调用模型。运行时 Skill Pack 是按 Operation/Role 锁定成员 Revision 的主辅助 Skill 绑定集合，不是 composite Prompt；`/novel new` 进入可消费 Skill Pack 的内置新建小说方案。完整契约见 [Agent Skill 与用户自定义 Skill 需求](./agent-skill-requirements.md)。
110. Agent Skill 创建质量采用用途与具体样例驱动，而不是一句描述直接落正式内容。Creator 必须形成正向触发样例、不适用边界和 adaptive/guided/strict 指导自由度，只保留非通用、可复用且可执行的方法；已有信息足够时不得强制完整表单或重复追问。User/Novel Skill 的语义选择默认 suggest，只有正反触发验证通过才允许 auto。运行时按索引元数据、核心方法、执行资源三级渐进加载，第一阶段不允许 Skill 脚本、二进制附件和远程运行时引用。来源提炼型 Skill 以留出样本或中性任务进行不泄露预期答案的独立验证；没有独立样本时只能报告结构验证。真实使用反馈只生成更新 Draft 和新 Revision，不改变旧 Run 或历史行为。完整契约见 [Agent Skill 与用户自定义 Skill 需求](./agent-skill-requirements.md)。
111. Agent 默认采用“自动工作”模式：只读探索、分析、计划修订以及可撤销 DraftSession/DraftBatch 的生成和重新生成无需形式化计划审批。`requestedEffect` 继续用于描述实际工作效果，但不再作为不可扩展的历史权限上限；计划按实际 Tool、Toolchain 和 deliverable 归一化效果，修订可以在报告和可审核草稿之间切换。分析范围、证据质量、创作方向和章节拍属于创作质量控制，在自动模式下使用已选范围、推荐方向或默认节拍继续执行并保留警告；“逐步审核”模式仍可显式启用这些检查点。角色只影响方法、推荐负责人和展示，不构成调用权限。工具白名单、参数 Schema、版本冲突、幂等、未知副作用对账继续后台强制执行。正文、大纲、角色等正式数据写回，覆盖、删除、批量提交和外部发送仍必须走审核或目标确认，普通计划不得直接包含正式写入工具。本决策取代决策 101、102、104 中关于计划不可扩权、只允许小范围只读自动执行和章节拍始终阻断的规定。
112. 外部网页研究暂缓实现。恢复时采用桌面端直连的 `ddgs` 搜索 provider，作为零 API Key、零本地服务、零 Gateway 的首版方案；不得要求用户安装 Docker、部署 SearXNG 或选择单条搜索结果。`ddgs` 是对 DuckDuckGo 公开搜索结果的非官方封装，不得表述为 DuckDuckGo 官方网页搜索 API，也不得承诺稳定性或结果覆盖。模型在全局外部研究能力启用时自动生成 2-4 条查询、每条最多读取 5 个结果、自动挑选公开 URL 并调用正文提取；不得使用代理轮换、绕过验证码或付费墙。限流、超时、空结果或正文抓取失败时，Toolchain 必须保留已获得的可追溯证据、明确报告外部资料不足并降级结论，不得伪造来源或改用项目内 `search.query` 冒充互联网结果。恢复时新增独立 `web.search` / `web.fetch` 工具，现有 `search.query` 永远只检索当前小说；外部来源仅保存查询、URL、资料类型、短证据、覆盖度、时间和内容哈希，不保存网页或作品全文。具体 Skill Creator 链路见 [Agent Skill 与用户自定义 Skill 需求](./agent-skill-requirements.md)。
113. 结构化模型输出采用单一权威契约治理，不允许 Electron 浅层 Schema、Python 深层模型和 Prompt 字段说明长期手工漂移。每个 `sourceMethod` 的权威契约必须派生双方校验器、修复 Schema、Prompt 约束和测试 Fixture；Provider 原生 Structured Outputs 只作为前置约束，本地完整校验仍是最终边界。恢复时合并本地与下游校验问题，只要仍有未解决问题就不得以“本地合法”为由跳过修复。安全的语法/确定性规范化先执行，模型修复最多自动一次、人工一次。完整分期见 [Agent 运行韧性、重试、降级与任务恢复需求](./retry-failover-resilience-requirements.md#931-结构化模型输出统一治理)。
114. JSON/Schema/Pydantic 是 Agent 平台内部协议，不是 Skill 或创作者的产品概念。任何 Skill、Toolchain、聊天、计划、报告、新建小说和多 Agent 综合只有在统一契约校验、领域校验、持久化和会话发布全部完成后才能显示成功；模型调用成功不等于任务完成。UI 不再使用“修复 JSON”作为主文案，统一呈现“修复结果并继续 / 重新生成本步骤 / 继续保存”等任务语言。新结构化能力未通过平台契约门禁前不得进入 `/` 面板、快捷按钮或自动路由。完整产品定义见 [Agent 产品方向](./product-direction.md#ai-结果可靠性与产品化恢复)。
115. Skill Creator 的目标生成内核改为 SQLite 虚拟文档式 Agent Authoring，不再把完整 Skill 或 Skill Pack 作为一次性大 JSON 模型响应，也不要求创建物理文件工作区。该能力不新增 Workspace/Document/Operation 表：现有 `AgentSkillDraft` 就是工作区，`SKILL.md` 和 references 作为逻辑文本文档保存在 `draftJson.authoring.documents`，Pack 标题与绑定继续使用现有小型 `draftJson.pack`，本阶段不新增 `PACK.yaml` 或 Pack 分发格式。方案复用已有 `status`、`version`、256 KiB 上限和原子发布事务；接入前先把 `upsertDraft(expectedVersion)` 从读取后比较加固为数据库条件更新 CAS。模型只能通过受控 `create/list/read/write/patch/remove/set_pack/validate/compile/diff` Tool 操作 Draft，并根据确定性逻辑路径/行级诊断做最小修订；不得获得通用 SQL、表名、数据库连接或正式 Skill 写权限。`patch` 首期只做带内容哈希的唯一精确文本替换，不实现通用 diff 或自动合并。文档或 Pack 变化后，以 `authoringInputHash/validatedInputHash/compiledInputHash` 一致性阻止旧领域投影被发布。验证通过后，用户查看 Diff 并确认，沿用当前提交事务发布不可变 Revision。结构化契约继续约束 Tool Call、诊断、状态和发布命令；数据库 Revision 仍是正式运行时真源。旧结构化 Creator Prompt、契约和路由在新链路切换后直接删除，不保留生成兼容入口。完整分期见 [Agent Skill 与用户自定义 Skill 需求](./agent-skill-requirements.md#117-sqlite-虚拟文档式-authoring-workspace)。
116. 文风 Skill Pack 的新任务主链只用结构化契约生成有界成员规划，不在协议中承载长正文；Runtime 先创建一个可恢复的 `AgentSkillDraft` Workspace，再按 `language-style/SKILL.md`、`suspense-release/SKILL.md` 和可选群像路径逐个调用通用文档作者并在每次写入后保存 Draft version。Pack 绑定通过 `set_pack` 单独写入。确定性校验失败时最多进行 4 轮，只把对应逻辑路径的诊断和当前文档交给模型，合法成员与来源读取结果不重做；验证成功后才 compile 并发布审核 Artifact。新任务使用 `agent.plan_style_skill_pack + agent.generate_skill_document`；旧 `agent.generate_style_skill_pack` Prompt、契约和路由删除，旧阶段 Run 不再恢复旧生成链。
...
- 跳过正式写回、覆盖、删除或外部发送确认的不可逆“完全控制”模式。
- 直接照搬 Codex App 的计划模式命令交互。
- Python 进程重启后不得重放身份不明的旧副作用调用；已绑定稳定 `operationKey/operationId` 的异步草稿 Run 可以从 LangGraph checkpoint 恢复并继续查询同一 Operation。
